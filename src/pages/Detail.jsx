  ...
  if (!data)
    return (
      <EmptyState
        ...
        action={<Link to="/items">목록으로 돌아가기</Link>}
      />
    );
  return (
    <>
      ...
      <article className="detail">
        ...
        <h1>{data.title}</h1>
        <p className="muted">
          작성 {new Date(data.created_at).toLocaleString('ko-KR')} · 수정{' '}
          {new Date(data.updated_at).toLocaleString('ko-KR')}
        </p>
        <Card>
          <div className="body-text">{data.content}</div>
        </Card>
        <div className="actions">
          <Link className="button" to={`/items/${id}/edit`}>
            수정하기
          </Link>
